def test_Simbox():
    ...
    assert np.all(simbox.get_lengths() == np.array([3,4,5])), "Problem with simbox.get_lengths()"

    ## Test dist_sq_dr_function
    ri = np.array([2.5,3,4])
    rj = np.array([0.5,0.5,0.5])
    dr = 0*ri
    result = simbox.get_dist_sq_dr_function()(ri, rj, simbox.get_lengths(), dr)
    ...
    result = simbox.get_dist_sq_function()(ri, rj, simbox.get_lengths())
    ...
    simbox.get_apply_PBC()(r, img, simbox.get_lengths())
    ...
    ## Test volume
    assert simbox.get_volume_function()(simbox.get_lengths())==3*4*5, "Problem with Simbox.volume"

    ## Test dist_moved_sq_function
    r_current = np.array([1,1,1])
    r_last    = r_current + 0.75*simbox.get_lengths()
    result = simbox.get_dist_moved_sq_function()(r_current, r_last, simbox.get_lengths(), simbox.get_lengths())
    ...
    skin, cut = 2.e0, 1.e0
    result = simbox.get_dist_moved_exceeds_limit_function()(r_current, r_last, simbox.get_lengths(), simbox.get_lengths(), skin, cut)
